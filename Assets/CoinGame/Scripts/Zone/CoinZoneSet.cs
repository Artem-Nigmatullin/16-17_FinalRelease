...
public class CoinZoneSet : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else if (_state == ZoneState.CompleteResponseKeyLockTrigger)
        {
            TakeCoin();
    ...
    public void OnPlayerEntered(CoinZone zone)
    {
        ...
        else if (_state == ZoneState.WaitingKeyLockTrigger)
        {
            ActivateKeyLockZone();
            DevLog.Log("state:" + _state);
        }
        else if (_state == ZoneState.Completed)
        {
            Complete();
            DevLog.Log("state:" + _state);
        }
